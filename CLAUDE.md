...
- `구분(선택)` is a **relation** to a separate "구분 색상" data source, not a select field. `구분` is a rollup mirroring it. The Notion integration used by this script must be explicitly shared with **both** the "대명 창고" data source and the "구분 색상" data source — if the integration only has access to the former, the API silently returns an empty relation array for `구분(선택)` (matching values are still visible in the Notion UI, which can be misleading when debugging). `resolveCalendar()` in `src/sync.js` logs a `console.warn` whenever a page's `categoryPageId` doesn't match any known category ID (as opposed to matching a known-but-intentionally-excluded one like 자기개발) — this is the signal to watch for in run logs if category IDs, tag names, or integration access ever drift, since otherwise the page is silently swept into "제외" indistinguishably from an intentional exclusion.
- Only pages with `Date >= (today - SYNC_CUTOFF_DAYS_AGO days)` are ever fetched by the sync direction, so past entries are structurally excluded rather than filtered after the fact. This cutoff is computed fresh on every run in `src/config.js` (`SYNC_CUTOFF_DAYS_AGO`, currently 30) rather than read from a fixed env var — it used to be a hardcoded `SYNC_CUTOFF_DATE` secret that never advanced, so the query scope grew without bound over time. One consequence of the rolling window: if a Notion page's `Date` ages past the cutoff, it drops out of scope entirely — including the delete-reconcile check in `sync.js` — so a previously-synced Google Calendar event for it is never cleaned up even if the Notion page is later deleted. In practice this doesn't matter for old, already-past events.
